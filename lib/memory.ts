...
export class MemoryManager {
  ...
  public async vectorSearch(
    ...
    const PineconeClient = <PineconeClient>(
      this.vectorDBClient
    );
    ...
    const similarDocs = await vectorStore
      ...
      .catch((err) => {
        console.log(
          'Failed to get vector search results',
          err,
        );
      });
  ...
  public async writeHistory(
    text: string,
    companionKey: CompanionKey,
  ) {
    if (
      !companionKey ||
      typeof companionKey.userId === 'undefined'
    ) {
    ...
    const key =
      this.generateRedisCompanionKey(companionKey);
